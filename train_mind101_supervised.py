...
from op.losses import NCCLoss, grad_loss3d, DiceLossMultiClass
...
def build_dataset(args):
    trans_data = [
            CenterCrop(dst_size=(160,192,160), rnd_offset=5),
            # CenterCrop(dst_size=(160,176,128), rnd_offset=5),
        ]
    trans_pair = [
        # RandomFlip(axes=[0,1,2])
    ]
    # data_list = sorted(os.listdir(args.train_data_root))[:40] 
    data_list = sorted(os.listdir(args.train_data_root)) 
    train_data = Mind101Dataset(args.train_data_root, data_list, trans=trans_data, with_label=True)
    train_data = PairDataset(train_data, None, trans_pair, with_label=True)
    val_data = None

    return train_data, val_data


def loss_fn(pred_warp, data_fix, label_warp, label_fix, flow, delta_list):
    loss_smi = NCCLoss(window=(9,9,9))(pred_warp, data_fix)
    loss_smooth = sum([ grad_loss3d(delta) for delta in delta_list ])
    loss_dice = DiceLossMultiClass(n_class=6)(label_warp, label_fix)
    loss_dice = 0.1*loss_dice
    # add dice loss
    return loss_smi, loss_smooth, loss_dice


def train(args, world_size, rank):    
    # make output folder
    output_dir = './output/mind101_supervised_%s_%s' % (args.net, args.tag)
    if rank == 0:
...
def parse_args():
    ...
    parser.add_argument("--val_data_root", help="validation data folder", type=str)
