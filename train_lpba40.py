...
def build_dataset(args):
    ...
    data_list = [ ("S%02d" % (i+1)) for i in range(30) ]
    train_data = LPBA40Dataset(args.train_data_root, data_list, trans=trans_data)
    train_data = PairDataset(train_data, None, trans_pair)
    val_data = None
...
def train(args, world_size, rank):    
    ...
    # make output folder
    output_dir = './output/lpba40_%s_%s' % (args.net, args.tag)
    if rank == 0:
    ...
    # build solver
    criterion = loss_fn
    #solver = AdversarialSolver(net_G, net_D, train_data, criterion, output_dir, world_size, rank)
