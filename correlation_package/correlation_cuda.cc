                       ...
                       int corr_type_multiply)
{
  ...
  int inputDepth = input1.size(2);
  int inputHeight = input1.size(3);
  int inputWidth = input1.size(4);

  int kernel_radius = (kernel_size - 1) / 2;
  int border_radius = kernel_radius + max_displacement;

  int paddedInputDepth = inputDepth + 2 * pad_size;
  int paddedInputHeight = inputHeight + 2 * pad_size;
  int paddedInputWidth = inputWidth + 2 * pad_size;

  int nOutputChannels = ((max_displacement/stride2)*2 + 1) * ((max_displacement/stride2)*2 + 1) * ((max_displacement/stride2)*2 + 1);

  int outputDepth = ceil(static_cast<float>(paddedInputDepth - 2 * border_radius) / static_cast<float>(stride1));
  ...
  rInput1.resize_({batchSize, paddedInputDepth, paddedInputHeight, paddedInputWidth, nInputChannels});
  rInput2.resize_({batchSize, paddedInputDepth, paddedInputHeight, paddedInputWidth, nInputChannels});
  output.resize_({batchSize, nOutputChannels, outputDepth, outputHeight, outputwidth});
                       ...
                       int corr_type_multiply)
{
  ...
  int batchSize = input1.size(0);
  int nInputChannels = input1.size(1);
  int paddedInputDepth = input1.size(2)+ 2 * pad_size;
  int paddedInputHeight = input1.size(3)+ 2 * pad_size;
  int paddedInputWidth = input1.size(4)+ 2 * pad_size;

  int depth = input1.size(2);
  int height = input1.size(3);
  int width = input1.size(4);

  rInput1.resize_({batchSize, paddedInputDepth, paddedInputHeight, paddedInputWidth, nInputChannels});
  rInput2.resize_({batchSize, paddedInputDepth, paddedInputHeight, paddedInputWidth, nInputChannels});
  gradInput1.resize_({batchSize, nInputChannels, depth, height, width});
  gradInput2.resize_({batchSize, nInputChannels, depth, height, width});
